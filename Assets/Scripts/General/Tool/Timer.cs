...
namespace UnityFramework.Runtime
{
    public class Timer
    {
        ...
        private float CurrentTime
        {
            get
            {
                return _ignorTimescale ? Time.realtimeSinceStartup : Time.time;
            }
        }
        /// <summary>
        /// 缓存时间
        ...
        {
            if (_isFinish)
                if (showLog) Log.Warning("【TimerTrigger（容错）】:计时已经结束！");
                else
                    _isPause = true;
        }
        /// <summary>  
        ...
        private void Resum()
        {
            if (_isFinish)
                ...
                else
                {
                    ...
                    {
                        if (showLog) Log.Warning("【TimerTrigger（容错）】:计时并未处于暂停状态！");
                    }
                }
        }
        ...
        public static void EndTime(string flag)
        {
            ...
            {
                //if (showLog) Log.Debug("【TimerTrigger（容错）】:定时器已完成触发或无此定时器！---Flag【" + flag + "】。");
            }
        }
        ...
        public static void UpdateAllTimer()
        {
            ...
            {
                if (null != timers[i] && !timers[i]._isFinish)
                {
                    timers[i].Update();
                }
                else
        ...
        public static void Pause(Timer timer)
        {
            ...
            {
                if (showLog) Log.Debug("【TimerTrigger（容错）】:此定时器已完成触发或无此定时器！");
            }
        }
        /// <summary>
        /// 恢复用户指定的计时触发器
        ...
        public static void DelTimer(Action completedEvent)
        {
            ...
            if (null != timer)
                ...
                if (showLog) Log.Debug("【TimerTrigger（容错）】:定时器已完成触发或无此定时器！---方法名：【" + completedEvent.Method.Name + "】。");
        }
        /// <summary>
        /// 删除用户指定的计时触发器
        /// </summary>
        ...
        public static void DelTimer(Action<float> updateEvent)
        {
            ...
            if (null != timer)
                ...
                if (showLog) Log.Debug("【TimerTrigger（容错）】:定时器已完成触发或无此定时器！---方法名：【" + updateEvent.Method.Name + "】。");
        }
        ...
        public Timer SetTime(float endTime)
        {
            ...
            else
            {
                if (endTime == _time)
                {
                    if (showLog) Log.Debug("【TimerTrigger（容错）】:时间已被设置，请勿重复操作！");
                }
                else
                {
                    if (endTime < 0)
                    {
                        if (showLog) Log.Debug("【TimerTrigger（容错）】:时间不支持负数，已自动取正！");
                        endTime = Mathf.Abs(endTime);
                    }
                    if (endTime < timePassed)//如果用户设置时间已错失
                    {
                        if (showLog) Log.Debug(string.Format("【TimerTrigger（容错）】:时间设置过短【passed:set=>{0}:{1}】,事件提前触发！", timePassed, endTime));
                    }
                    _time = endTime;
                }
            }
        ...
        public Timer Setloop(bool loop)
        {
            ...
            {
                if (showLog) Log.Debug("【TimerTrigger（容错）】:定时器已失效,设置Loop Fail！");
            }
            return this;
        }
        /// <summary>
        ...
        public Timer SetIgnoreTimeScale(bool ignoreTimescale)
        {
            ...
            {
                if (showLog) Log.Debug("【TimerTrigger（容错）】:定时器已失效，设置IgnoreTimescale Fail！");
            }
            return this;
        }
        #endregion
    ...
    public static class TimerExtend
    {
        ...
        /// 等待指定时间简化版
        /// </summary>
        /// <param name="Self">自身组件</param>
        /// <param name="Time">等待时长</param>
        /// <param name="callBack">回调</param>
        public static void WaitTime(this Component Self, float Time, Action callBack)
        {
            var saveName = Self.name;
            Timer.AddTimer(Time, saveName).OnCompleted(() =>
            {
                callBack.Invoke();
                Timer.DelTimer(saveName);
            });
            //原本使用dotween等待 有问题在切回去
            //    float index = 0;
            //    DOTween.To(() => index, x => index = x, 1, Time).onComplete = () => callBack.Invoke();
        }
        /// <summary>
        /// 当计时器计数完成时执行的事件链
        /// </summary>
