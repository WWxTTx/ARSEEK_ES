...
namespace UnityEngine.UI.Extensions
{
    ...
    public class UILineRenderer : UIPrimitiveBase
    {
        ...
        [SerializeField, Tooltip("Arror Segments to be drawn\n This is a list of arrays of points")]
        internal Vector2[] m_arrow;
        ...
        private void CreateTriangle(VertexHelper vh, Vector2 startWorldPos, Vector2 endWorldPos)
        {
            var dir = (endWorldPos - startWorldPos).normalized;

            Vector2 point = endWorldPos + dir * lineThickness * 2;

            int v0 = AddVert(vh, point, new Vector2(0f, 0f));
            int v1 = AddVert(vh, (point - RotateBy(dir, 20) * lineThickness * 4), new Vector2(0f, 1f));
            int v2 = AddVert(vh, (point - RotateBy(dir, -20) * lineThickness * 4), new Vector2(1f, 1f));
        ...
        private Vector2 RotateBy(Vector2 v, float angle)
        {
            ...
            var ca = System.Math.Cos(angle);
            var sa = System.Math.Sin(angle);
            var rx = v.x * ca - v.y * sa;

            return new Vector2((float)rx, (float)(v.x * sa + v.y * ca));
        }

        private int AddVert(VertexHelper vh, Vector2 worldPos, Vector2 uv)
        {
            vh.AddVert(worldPos, color, uv);
