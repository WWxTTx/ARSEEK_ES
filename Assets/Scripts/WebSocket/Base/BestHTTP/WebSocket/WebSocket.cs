        ...
        private void OnInternalRequestCallback(HTTPRequest req, HTTPResponse resp)
        {
            ...
            switch (req.State)
            {
                ...
                case HTTPRequestStates.Error:
                    reason = "Request Finished with Error! " + (req.Exception != null ? ("Exception: " + req.Exception.GetType() + req.Exception.Message + req.Exception.StackTrace) : string.Empty);
                    if (req.Exception.GetType() == typeof(System.Net.Sockets.SocketException))
                        reason += " ErrorCode:" + ((System.Net.Sockets.SocketException)req.Exception).ErrorCode;
                    break;
