...
public class InfiniteListScrollRectInspector : ScrollRectEditor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        GameObject lElement = EditorGUILayout.ObjectField("Element Template", _target.ElementTemplate, typeof(GameObject), true) as GameObject;
        if (EditorGUI.EndChangeCheck())
        {
            Undo.RecordObject(target, "Set InfiniteList ElementTemplate");
            _target.ElementTemplate = lElement;
            EditorUtility.SetDirty(target);
        }

        EditorGUI.BeginChangeCheck();
        GameObject rElement = EditorGUILayout.ObjectField("Self Element Template", _target.SelfElementTemplate, typeof(GameObject), true) as GameObject;
        if (EditorGUI.EndChangeCheck())
        {
            Undo.RecordObject(target, "Set InfiniteList SelfElementTemplate");
            _target.SelfElementTemplate = rElement;
            EditorUtility.SetDirty(target);
        }

        //EditorGUI.BeginChangeCheck();
        //_target.ListElementType = (InfiniteDoubleListElementType)EditorGUILayout.EnumPopup(_target.ListElementType);
        //if (EditorGUI.EndChangeCheck())
        //{
