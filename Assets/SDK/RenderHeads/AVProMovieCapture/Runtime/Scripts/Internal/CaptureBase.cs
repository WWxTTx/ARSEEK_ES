...
namespace RenderHeads.Media.AVProMovieCapture
{
	...
	public partial class CaptureBase : MonoBehaviour
	{
		...
		public enum StopCaptureMode
		{
			Manual,
			DiskLeak,
			SaveFailed,
			DiskLeakAndSaveFailed
		}
		...
		// Start Capture Failed Reason
		StartFailedMode _startFailedReason = StartFailedMode.Unknown;

		public StartFailedMode StartFailedReason
        {
			get { return _startFailedReason; }
        }

		// Stop Capture Reason
		public StopCaptureMode StopCaptureReason = StopCaptureMode.Manual;

		public UnityEvent<StopCaptureMode> OnRecordComplete = new UnityEvent<StopCaptureMode>();
		...
		public virtual bool PrepareCapture()
		{
			...
			if (_minimumDiskSpaceMB > 0 && _outputTarget == OutputTarget.VideoFile)
			{
				...
				if (!IsEnoughDiskSpace())
				{
					...
					_startFailedReason = StartFailedMode.DiskLeak;
					return false;
				}
			}
#endif
				...
				else if (_audioCaptureSource == AudioCaptureSource.Unity || _audioCaptureSource == AudioCaptureSource.Wwise)
				{
					...
					if (_unityAudioCapture != null)
					{
						...
						if (_audioCaptureSource == AudioCaptureSource.Unity)
						{
							if (_isRealTime)
							{
								//CaptureAudioFromMultipleSources
			...
			else
			{
                ...
                {
					_startFailedReason = StartFailedMode.AccessDenied;
                }
#endif
...
// (mac|i)OS only for now
		...
		{
			StopCaptureReason = StopCaptureMode.Manual;
		...
		private static MP4FileProcessing.Options CreatePostOperationsOptions(VideoEncoderHints hints, string finalFilePath)
		{
			...
			// 尝试解决内存不足时停止录制文件写入异常
			options.applyFastStart = false;// hints.allowFastStartStreamingPostProcess;
			#endif
			...
			if (CanApplyPostOperations(filePath, hints, finalFilePath))
			{
				try
				{
					...
					if (!MP4FileProcessing.ProcessFile(filePath, false, options))
					{
						...
						if (StopCaptureReason == StopCaptureMode.DiskLeak)
							StopCaptureReason = StopCaptureMode.SaveFailed;
						else
							StopCaptureReason = StopCaptureMode.DiskLeakAndSaveFailed;
						OnRecordComplete?.Invoke(StopCaptureReason);
					}
				}
		...
		private void CheckFreeDiskSpace()
		{
			if (_minimumDiskSpaceMB > 0)
			{
				if (!IsEnoughDiskSpace())
				{
					...
					StopCaptureReason = StopCaptureMode.DiskLeak;
					//StopCapture(true);
					// 尝试解决内存不足时停止录制文件写入异常
					StopCapture();
				}
