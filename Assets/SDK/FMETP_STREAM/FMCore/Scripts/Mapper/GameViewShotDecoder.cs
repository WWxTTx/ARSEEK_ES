...
public class GameViewShotDecoder : MonoBehaviour
{
    ...
    public UnityEventUIDTexture2D OnReceivedTexture2D = new UnityEventUIDTexture2D();
    ...
    IEnumerator ProcessImageData(byte[] _byteData)
    {
        ...
        {
            if (ReceivedTexture2D.format != textureFormat)
            {
        ...
        if (ReceivedTexture2D == null) ReceivedTexture2D = new Texture2D(0, 0, textureFormat, false);
        ...
        if (FastMode)
        {
            ...
            if (AsyncMode && Loom.numThreads < Loom.maxThreads)
            {
                ...
                Loom.RunAsync(() =>
                {
                    try { _byteData.FMJPGToRawTextureData(ref RawTextureData, ref _width, ref _height, textureFormat); }
                    catch { }
                    AsyncDecoding = false;
        ...
        OnReceivedTexture2D.Invoke($"{label}_{dataID}",ReceivedTexture2D);
