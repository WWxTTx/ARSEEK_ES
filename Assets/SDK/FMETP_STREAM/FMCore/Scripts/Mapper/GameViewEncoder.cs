...
public enum GameViewCaptureMode { RenderCam, MainCam, FullScreen, Desktop, Rect }
...
public class GameViewEncoder : MonoBehaviour
{
    ...
    public Vector2 RectOffsetPosition;
    ...
    public float MinStreamFPS = 1f;
    private float minStreamInterval;
    private float minNext = 0f;
    ...
    public int chunkSize { get; set; } = 65536; // 1048576;// 8096; //32768
    ...
    private void Start()
    {
        ...
        minStreamInterval = 1f / MinStreamFPS;

        CaptureModeUpdate();
        CheckResolution();

        //if (sendCOR == null)
        //    sendCOR = StartCoroutine(SenderCOR());
    }
    ...
    private void Update()
    {
        ...
        switch (_CaptureMode)
        {
            ...
            case GameViewCaptureMode.FullScreen:
                renderResolution = new Vector2(Screen.width, Screen.height) / Mathf.Pow(2, (int)Resize);
                break;
            case GameViewCaptureMode.Rect:
                renderResolution = (new Vector2(Screen.width, Screen.height) - RectOffsetPosition) / Mathf.Pow(2, (int)Resize);
                break;
    ...
    void CheckResolution()
    {
        ...
        bool IsLinear = (ColorSpace == ColorSpace.Linear) && (CaptureMode == GameViewCaptureMode.FullScreen || CaptureMode == GameViewCaptureMode.Rect);
    ...
    IEnumerator RenderTextureRefresh()
    {
        if (NeedUpdateTexture && !EncodingTexture)
        {
            ...
            if (_CaptureMode == GameViewCaptureMode.Rect)
            {
                //if (Resize == GameViewResize.Full)
                //{
                //    // cleanup
                //    if (CapturedTexture != null) Destroy(CapturedTexture);
                //    CapturedTexture = new Texture2D(Screen.width - (int)RectOffsetPosition.x, Screen.height - (int)RectOffsetPosition.y, TextureFormat.RGB24, false);
                //    CapturedTexture.ReadPixels(new Rect(RectOffsetPosition.x, 0, Screen.width - RectOffsetPosition.x, Screen.height - RectOffsetPosition.y), 0, 0, false);
                //    CapturedTexture.Apply();
                ...
                //else
                {
                    // cleanup
                    if (Screenshot != null) Destroy(Screenshot);
                    Screenshot = new Texture2D(Screen.width - (int)RectOffsetPosition.x, Screen.height - (int)RectOffsetPosition.y, TextureFormat.RGB24, false);
                    Screenshot.ReadPixels(new Rect(RectOffsetPosition.x, 0, Screen.width - RectOffsetPosition.x, Screen.height - RectOffsetPosition.y), 0, 0, false);
                    Screenshot.Apply();
    ...
    IEnumerator EncodeBytes(byte[] RawTextureData = null)
    {
        if (CapturedTexture != null || RawTextureData != null)
        {
            ...
            {
                frameOperation = NetworkManager.Instance.GetVideoPacket();
                opByte = Encoding.UTF8.GetBytes(frameOperation);
            }

            if (ignoreSimilarTexture && (opByte == null || opByte.Length == 0 ) && Time.realtimeSinceStartup < minNext)
            {
            ...
            minNext = Time.realtimeSinceStartup + minStreamInterval;
            ...
            if (opByte == null)
            {
                ...
                for (int i = 0; i <= chunks; i++)
                {
                    ...
                    Buffer.BlockCopy(_meta_id, 0, SendByte, 0, 4);//4,4
                    Buffer.BlockCopy(_meta_length, 0, SendByte, 4, 4);//8,4

                    Buffer.BlockCopy(_meta_offset, 0, SendByte, 8, 4);//12,4
                    SendByte[12] = (byte)(GZipMode ? 1 : 0);//16
            ...
            {
                int chunks = Mathf.RoundToInt(dataByte.Length / chunkSize) + 1;
                for (int i = 0; i <= chunks; i++)
                {
                    int SendByteLength = (i == 0) ? (15 + opByte.Length) : (i == chunks) ? (_length % chunkSize + 15) : (chunkSize + 15);//19
                    byte[] _meta_offset = BitConverter.GetBytes(_offset);
                    byte[] SendByte = new byte[SendByteLength];
                    ...
                    SendByte[12] = (byte)(GZipMode ? 1 : 0);//16
                    SendByte[13] = (byte)ColorReductionLevel;//17
                    SendByte[14] = (byte)(i == 0 ? 1 : 0);//18

                    if (i == 0)
                    {
                        Buffer.BlockCopy(opByte, 0, SendByte, 15, SendByte.Length - 15);//19
                    }
                    else
                    {
